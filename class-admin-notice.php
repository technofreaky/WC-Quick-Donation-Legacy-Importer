...
class WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notices {
    ...
    private function __construct() {
        self::$instance = null;
        $this->noticesArrayName = 'wc_qd_li_AdminNotices';
        $this->REQUESTID = 'wcqdlsmsg';       
        $this->notices = array();
     ...
     */
    public function addNotice(WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notice $notice) {
        $this->notices[] = $notice;
        $this->storeNotices();
    }
     ...
     * @param WooCommerce_Quick_Donation_Admin_Notice $notice
     * @return bool 
     */
    private function isTimeToDisplay(WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notice $notice) {
 ...
 */
abstract class WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notice {
    protected $content;
    protected $type;
    protected $screen;
    protected $id;
    ...
    public function getContentFormated($wrapInParTag = true) {
        ...
        $before = '<div id="wc_qd_li_notice_'.$this->id.'"   class="' .$class. '">';
     ...
     * @return \WooCommerce_Quick_Donation_Admin_Notice
     */
 ...
 * Type of notices
 */
class WooCommerce_Quick_Donation_Legacy_Importer_Admin_Error_Notice extends WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notice { protected $type = 'error'; }
class WooCommerce_Quick_Donation_Legacy_Importer_Admin_Updated_Notice extends WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notice { protected $type = 'updated'; }
class WooCommerce_Quick_Donation_Legacy_Importer_Admin_UpdateNag_Notice extends WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notice { protected $type = 'update-nag'; }
 


if(!function_exists('wc_qd_li_notice')){
    function wc_qd_li_notice( $message, $type = 'update',$args = array()) {
        $notice = ''; 
        $defaults = array('times' => 1,'screen' => array(),'users' => array(), 'wraper' => true,'id'=>'');    
        $args = wp_parse_args( $args, $defaults );
        extract($args);
        if($type == 'error'){$notice = new WooCommerce_Quick_Donation_Legacy_Importer_Admin_Error_Notice($message,$id,$times, $screen, $users);}
        if($type == 'update'){ $notice = new WooCommerce_Quick_Donation_Legacy_Importer_Admin_Updated_Notice($message,$id,$times, $screen, $users); }
        if($type == 'upgrade'){ $notice = new WooCommerce_Quick_Donation_Legacy_Importer_Admin_UpdateNag_Notice($message,$id,$times, $screen, $users); } 
        $msgID = $notice->getId();
        $message = str_replace('$msgID$',$msgID,$message);
        $notice->setContent($message);
        $notice->setWrapper($wraper);
        WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notices::getInstance()->addNotice($notice);
    }
}

if(!function_exists('wc_qd_li_remove_notice')){
    function wc_qd_li_remove_notice($id){
        WooCommerce_Quick_Donation_Legacy_Importer_Admin_Notices::getInstance()->deleteNotice($id);
        return true;
    }
}

if(!function_exists('wc_qd_li_admin_error')){
    function wc_qd_li_admin_error( $message,$times = 1,$id = '',$screen = array(),$args = array()) {
        $args['id'] = $id;
        $args['times'] = $times;
        $args['screen'] = $screen;
        wc_qd_li_notice($message,'error',$args);
    }
}

if(!function_exists('wc_qd_li_admin_update')){
    function wc_qd_li_admin_update( $message,$times = 1,$id = '',$screen = array(),$args = array()) {
        $args['id'] = $id;
        $args['times'] = $times;
        $args['screen'] = $screen;
        wc_qd_li_notice($message,'update',$args);
    }
}

if(!function_exists('wc_qd_li_admin_upgrade')){
    function wc_qd_li_admin_upgrade( $message,$times = 1,$id = '',$screen = array(),$args = array()) {
        $args['id'] = $id;
        $args['times'] = $times;
        $args['screen'] = $screen;
        wc_qd_li_notice($message,'upgrade',$args);
    }
}